...
let graphicsOrderReturnId: string[] = []
...
describe('Get graphicsOrder (end-to-end)', () => {
  ...
  test('should list all graphicsOrderReturn', async () => {
    graphicsOrderReturnId.push(graphicsOrderReturn.id)

    const response = await request(app)
      .get('/api/graphicsOrderReturn')
      .auth(jwt.token, { type: 'bearer' })

    expect(response.status).toBe(StatusCodes.OK)

    const OrderReturn = await graphicsOrderReturnRepository.list()
    expect(OrderReturn.length > 0).toBeTruthy()
  })

  test('should not be able to list graphicsOrderReturn without authentication', async () => {
    const response = await request(app).get(`/api/graphicsOrderReturn`).send()
