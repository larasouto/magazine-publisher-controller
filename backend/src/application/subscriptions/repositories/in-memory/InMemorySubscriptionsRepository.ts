...
export class InMemorySubscriptionsRepository implements ISubscriptionsRepository {
  ...
  async exists(id: string): Promise<boolean> {
    const subscription = this.subscriptions.some((subscription) => subscription.id === id)
    return !!subscription
  }
