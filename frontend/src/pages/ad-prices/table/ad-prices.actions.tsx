...
import { AdPricesColumns } from './ad-prices.columns'

type AdPricesActionsProps = {
  row: AdPricesColumns
}

export const AdPricesActions = ({ row }: AdPricesActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<AdPricesColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.adPrices.edit, [row.id])}
                color="foreground"
