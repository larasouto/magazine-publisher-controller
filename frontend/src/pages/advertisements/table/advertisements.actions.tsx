...
import { AdvertisingColumns } from './advertisements.columns'


type AdvertisementsActionsProps = {
  row: AdvertisingColumns
}

export const AdvertisementsActions = ({ row }: AdvertisementsActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<AdvertisingColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.advertisements.edit, [row.id])}
                color="foreground"
