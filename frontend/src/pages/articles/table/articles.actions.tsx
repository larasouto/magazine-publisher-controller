...
import { ArticleColumns } from './articles.columns'

type ArticlesActionsProps = {
  row: ArticleColumns
}

export const ArticlesActions = ({ row }: ArticlesActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<ArticleColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.articles.edit, [row.id])}
                color="foreground"
