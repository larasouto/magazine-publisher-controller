...
import { BookstoresColumns } from './bookstores.columns'

type BookstoresActionsProps = {
  row: BookstoresColumns
}

export const BookstoresActions = ({ row }: BookstoresActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<BookstoresColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.bookstores.edit, [row.id])}
                color="foreground"
