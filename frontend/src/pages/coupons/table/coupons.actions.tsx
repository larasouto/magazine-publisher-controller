...
type CouponsActionsProps = {
  row: CouponsColumns
}

export const CouponsActions = ({ row }: CouponsActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { removeMany } = useFetch<CouponsColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.coupons.edit, [row.id])}
                color="foreground"
