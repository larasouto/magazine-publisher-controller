...
import { DistributorsColumns } from './distributor.columns'

type DistributorActions = {
  row: DistributorsColumns
}

export const DistributorActions = ({ row }: DistributorActions) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<DistributorsColumns>({
    baseUrl: backend.distributor.baseUrl,
    query: ['distributorForm'],
