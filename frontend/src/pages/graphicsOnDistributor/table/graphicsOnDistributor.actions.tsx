...
import {  GraphocsOnDistributorsColumns } from './graphicsOnDistributor.columns'

type  GraphocsOnDistributorsActionsProps = {
  row:  GraphocsOnDistributorsColumns
}

export const  GraphocsOnDistributorsActions = ({ row }:  GraphocsOnDistributorsActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch< GraphocsOnDistributorsColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.graphicsOnDistributor.edit, [row.id])}
                color="foreground"
