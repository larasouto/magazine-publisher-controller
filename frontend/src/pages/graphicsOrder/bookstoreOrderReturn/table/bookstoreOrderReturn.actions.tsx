...
import { BookstoreOrderReturnsColumns } from './bookstoreOrderReturnv.columns'

type BookstoreOrderReturnsActionsProps = {
  row: BookstoreOrderReturnsColumns
}

export const BookstoreOrderReturnsActions = ({ row }: BookstoreOrderReturnsActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { remove } = useFetch<BookstoreOrderReturnsColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.bookstoreOrderReturn.edit, [row.id])}
                color="foreground"
