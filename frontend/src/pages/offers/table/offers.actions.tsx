...
type OffersActionsProps = {
  row: OffersColumns
}

export const OffersActions = ({ row }: OffersActionsProps) => {
  const { t } = useTranslation()
  const { isOpen, onOpen, onOpenChange } = useDisclosure()

  const { removeMany } = useFetch<OffersColumns>({
        ...
        <DropdownMenu aria-label="dropdown reporter">
          <DropdownSection title={t('table.actions')}>
            <DropdownItem textValue="edit">
              <Link
                href={replaceParams(routes.offers.edit, [row.id])}
                color="foreground"
