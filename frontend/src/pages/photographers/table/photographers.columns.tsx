...
import { ReporterActions } from './photographers.actions'

export type PhotographerColumns = {
  id: string
  name: string
  email: string
  phone?: string
...
const helper = createColumnHelper<PhotographerColumns>()
...
export const columns = [
  ...
  helper.accessor((row) => row.name, {
    id: 'name',
    header: () => t('photographers:form.name.label'),
    cell: ({ row }) => {
  ...
  helper.accessor((row) => row.cpf, {
    id: 'cpf',
    header: () => t('photographers:form.cpf.label'),
  ...
  helper.accessor((row) => row.entryDate, {
    id: 'entryDate',
    header: () => t('photographers:form.entry_date.label'),
    cell: ({ row }) => {
  ...
  helper.accessor((row) => row.departureDate, {
    id: 'departureDate',
    header: () => t('photographers:form.departure_date.label'),
    cell: ({ row }) => {
      const date = new Intl.DateTimeFormat(i18next.language)
  ...
  helper.accessor((row) => row.status, {
    id: 'status',
    header: () => t('photographers:form.status.label'),
    cell: ({ row }) => {
      ...
      return (
        <Chip color={colors[status]}>
          {t(`photographers:form.status.options.${status.toLowerCase()}`)}
        </Chip>
      )
    },
    enableSorting: true,
